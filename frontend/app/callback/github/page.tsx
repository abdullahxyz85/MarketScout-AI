...
export default function GithubCallbackPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/auth/github/callback?${query}`, {
      credentials: 'include',
    })
      .then(async (res) => {
        if (!res.ok) {
          const body = await res.json().catch(() => null);
          throw new Error(body?.detail ?? 'GitHub authentication failed');
        }
        return res.json();
      })
      .then((data) => {
      ...
      >
        {error ? (
          ...
          <div>
            ...
            <p className="text-white/50 text-sm">Signing you in with GitHub...</p>
          </div>
        )}
      </motion.div>
    </div>
