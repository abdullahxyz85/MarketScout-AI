...
export default function ResearchPage() {
  ...
  const graphContainerRef = useCallback((el: HTMLDivElement | null) => {
    ...
    if (!el) return;
    const updateSize = () =>
      setGraphSize({ width: el.clientWidth, height: 500 });
    updateSize();
        ...
        {stage === "done" && result && (
          ...
          >
            ...
            <GlassCard className="border-emerald-500/30">
              <GlassCardContent className="text-center py-8">
                ...
                <div className="flex gap-3 justify-center flex-wrap">
                  {jobId && (
                    <a
                      href={`/api/agents/research/${jobId}/report/pdf`}
                      target="_blank"
              ...
              {[
                {
                  ...
                  value: `${result?.innovation_score?.innovation_score ?? 0}/100`,
                  color: "text-indigo-400",
                },
                {
                  label: "Market Score",
                  value: `${result?.report?.market_score ?? 0}/100`,
                  color: "text-emerald-400",
                },
                {
                  label: "Opportunity",
                  value: `${result?.report?.opportunity_score ?? 0}/100`,
                  color: "text-purple-400",
                },
                {
                  label: "Competition",
                  value:
                    (result?.report?.competition_level ?? "medium")
                      .charAt(0)
                      .toUpperCase() +
                    (result?.report?.competition_level ?? "medium").slice(1),
                ...
                >
                  <GlassCard>
                    <GlassCardContent className="text-center py-5">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {result?.report?.key_metrics &&
                Object.keys(result.report.key_metrics).length > 0 && (
                  <GlassCard>
                    <GlassCardHeader>
                      <div className="flex items-center gap-2">
                    ...
                    <GlassCardContent>
                      <div className="space-y-3">
                        {Object.entries(result.report.key_metrics).map(
                          ([k, v]) => (
                            <div
                              key={k}
                              className="flex gap-4 text-sm border-b border-white/5 pb-2"
                            >
                              <span className="text-white/50 capitalize w-28 flex-shrink-0">
                                {k.replace(/_/g, " ")}
                              </span>
                  ...
                  <GlassCardContent>
                    <ul className="space-y-2">
                      {(result.report!.recommendations ?? []).map((rec, i) => (
                        ...
                        >
                          <span className="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-300 text-xs flex items-center justify-center flex-shrink-0 mt-0.5">
                            {i + 1}
                          </span>
                          {rec}
                        </li>
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10">
              ...
              <span className="text-sm font-semibold capitalize text-amber-400">
                {result?.risks?.overall_risk_level ?? "medium"}
              </span>
            </div>
              ...
              <GlassCardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setScenario((s) => ({
                          ...s,
                          pricing_strategy: e.target.value,
                        }))
                      }
                ...
                {scenarioResult && (
                  <div className="mt-4 p-4 rounded-xl bg-white/5 border border-white/10 space-y-3">
                    ...
                    {Array.isArray(scenarioResult.key_action_items) &&
                      scenarioResult.key_action_items.length > 0 && (
                        <ul className="space-y-1.5">
                          {scenarioResult.key_action_items.map(
